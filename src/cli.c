...
static void print_usage(void) {
    ...
    print_markup(
        ...
        "            README.md for all six -- \001--policy rewrite\001 is different: it never\n"
        ...
        "            instead of failing. \001--force\001 allows a \002<source>\002/\002<fallback>\002\n"
        "            path (not a bare name) that doesn't exist yet; \001doctor\001 skips its\n"
        "            existence check for whichever of them still doesn't, but dispatch\n"
        "            always checks for real when the shim actually runs. The shim's\n"
        "            confirmation line is colored; the shell-startup-file PATH-update\n"
        "            notices right after it are silent unless \001-v\001/\001--verbose\001 is given, or\n"
        "            \001verbose = true\001 is set as a top-level config default (config\n"
        "            defaults to \001false\001). A trial run that takes too long or produces\n"
        "            too much output gives up on being hideable and streams live instead\n"
        "            (nothing lost, just no more fallback for that run) -- tune this with\n"
        "            \001--capture-timeout\001/\001--capture-limit\001 (default 2000ms/8MiB; also\n"
        "            settable globally in the config). \001--capture-limit\001 takes a plain\n"
        "            byte count or a size with a unit suffix (B, K/KB, KiB, M/MB, MiB,\n"
        "            G/GB, GiB). \001--split-config\001 saves this shim's settings in their\n"
        "            own \002<name>\002-config.toml file (in the config directory) instead of\n"
        "            inside config.toml -- move that file to the shimback binary's own\n"
        ...
        "            anything's wrong. \001doctor fix\001 recreates any missing or dangling\n"
